...
CENTERING_GAIN = 0.75  # How strongly the car corrects its position toward the center of the track.
BRAKE_THRESHOLD = 0.10  # Angle threshold for braking. Lower values brake earlier.
GEAR_SPEEDS = [0, 45, 85, 125, 170, 220]  # Speed thresholds for gear shifting.
ENABLE_TRACTION_CONTROL = True  # Toggle traction control system.

LAST_STEER_ERROR = 0.0
STEER_ERROR_SUM = 0.0
...
STEER_INTEGRAL_LIMIT = 0.25

FILTERED_TRACK_POS = 0.0
KALMAN_GAIN_TRACK = 0.50

# ================= HELPER FUNCTIONS =================
def calculate_steering(S):
    global LAST_STEER_ERROR, FILTERED_TRACK_POS

    FILTERED_TRACK_POS = kalman_filter(
        S['trackPos'],
        FILTERED_TRACK_POS,
        KALMAN_GAIN_TRACK
    )
    ...
    angle = S['angle']
    track_pos = FILTERED_TRACK_POS

    front, side_diff, wide_diff, front_clear, fast_corner = get_curve_ahead(S)
    ...
    if fast_corner and angle > 0.04:
        target_pos = -0.65
    elif fast_corner and angle < -0.04:
        target_pos = 0.65

    base_steer = (angle * STEER_GAIN / math.pi) - ((track_pos - target_pos) * CENTERING_GAIN)

    error = base_steer
    derivative = error - LAST_STEER_ERROR

    correction = (KP_STEER * error) + (KD_STEER * derivative)

    steer = (base_steer * 0.85) + (correction * 0.15)

    if abs(angle) < 0.06 and abs(track_pos) < 0.20 and side_diff < 90:
        steer *= 0.25
...
def calculate_throttle(S, R):
    target_speed = get_target_speed(S)
    angle = abs(S['angle'])
    track_pos = abs(S['trackPos'])
    front, side_diff, wide_diff, front_clear, fast_corner = get_curve_ahead(S)

    if S['speedX'] < target_speed - (abs(R['steer']) * 1.5):
        accel = min(1.0, R['accel'] + 0.5)
    else:
        accel = max(0.0, R['accel'] - 0.2)

    if angle > 0.10 and angle < 0.40 and track_pos < 0.70 and front > 40:
        accel = max(accel, 0.25)

    if angle > 0.07 and angle < 0.28 and track_pos < 0.60 and front > 55:
        accel = max(accel, 0.40)

    if angle > 0.04 and angle < 0.18 and track_pos < 0.50 and front > 70:
        accel = max(accel, 0.55)

    if angle > 0.12 and angle < 0.35 and track_pos < 0.80 and front > 45:
        accel = max(accel, 0.50)
...
def get_curve_ahead(S):
    track = S['track']

    front = track[9]

    left_front = track[6]
    right_front = track[12]

    left_wide = track[4]
    right_wide = track[14]

    side_diff = abs(left_front - right_front)
    wide_diff = abs(left_wide - right_wide)

    front_clear = min(track[8], track[9], track[10])

    fast_corner = front_clear > 95 and wide_diff < 140

    return front, side_diff, wide_diff, front_clear, fast_corner
...
def get_target_speed(S):
    ...
    if front > 140 and side_diff < 40 and angle < 0.12:
        return 220

    if front > 120 and side_diff < 50 and angle < 0.16:
        return 215

    if front > 100 and side_diff < 60 and angle < 0.20:
        return 210

    if front_clear > 120 and wide_diff < 120 and angle < 0.18:
        return 220

    if side_diff > 190 and wide_diff > 160:
        return 125
    elif side_diff > 165 and wide_diff > 130:
        return 160
    elif side_diff > 135 and wide_diff > 100:
        return 190
    elif side_diff > 110:
        return 215
...
def apply_brakes(S):
    ...
    if fast_corner and angle < 0.30 and speed < 220:
        return 0.0

    brake = 0.0

    if front < 90 and speed > 155 and not fast_corner:
        brake += 0.15

    if front < 75 and speed > 115:
        brake += 0.15

    if front < 60 and speed > 95:
        brake += 0.20

    if front < 45 and speed > 80:
        brake += 0.20

    if front < 32 and speed > 65:
        brake += 0.20

    if angle > 0.18 and speed > 125:
        brake += 0.08

    if angle > 0.22 and speed > 100:
        brake += 0.10

    if angle > 0.32 and speed > 85:
        brake += 0.15

    if side_diff > 170 and wide_diff > 150 and speed > 125 and not fast_corner:
        brake += 0.10

    if side_diff > 210 and wide_diff > 180 and speed > 105 and not fast_corner:
        brake += 0.10

    if track_pos > 0.80 and speed > 55:
        brake += 0.20

    return max(0.0, min(0.75, brake))

def kalman_filter(value, filtered_value, gain):
    return filtered_value + gain * (value - filtered_value)
